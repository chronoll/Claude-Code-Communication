...
echo ""
echo "  2. 🤖 Agent CLI起動 (プロファイルは config/agent_cli.env で設定):"
echo "     # 手順1: President起動"
echo "     tmux send-keys -t president './bin/agent-launch.sh president' C-m"
echo "     # 手順2: multiagent一括起動"
echo "     # 各ペインのIDを使用して起動"
echo "     tmux list-panes -t multiagent:agents -F '#{pane_id}' | while read pane; do"
echo "         tmux send-keys -t \"\$pane\" './bin/agent-launch.sh' C-m"
echo "     done"
echo ""
echo "  3. 📜 指示書確認:"
